...
class LoginController extends ControladorBase {
  ...
  public function validar(){
    ...
    if( isset($_POST['nick_user']) && isset($_POST['pswd_user']) && !empty(trim($_POST['nick_user'])) && !empty(trim($_POST['pswd_user']))) :
     ...
     $datasUser = $objUsuario->getDatasUsuario($usuario[0] ->id);
//var_dump($datasUser);die;
      //Comprobar si hay datos del usuario 
      if ( !is_bool($usuario) ){
          //Hacemos el hash de la password y el token
          $user_pwd_success = $this->secury->hashing($pswd_user, $usuario[0]->password);
          $user_tkn_success = $this->secury->hashing($nick_user.$pswd_user, $usuario[0]->remember_token);

          //Validamos que el hash retorne 1 para aceptar el usuario
          if ( $user_pwd_success==1 && $user_tkn_success==1 && $datasUser->edo==9) {
              ...
              setcookie ("us_FUA", $_SESSION['usuario']['nick'], 0, "/"," ",0);
          } else
            $process = FALSE;
      } else
        $process = FALSE; 
